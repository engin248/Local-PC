...
impl CheckpointManager {
    pub fn verify_checkpoint(task_id: &str, node_id: Option<&str>, checkpoint_type: &str, is_valid: bool) -> Result<(), String> {
        let db = Database::new();
        ...
        let result_msg = if valid {
            format!("Kontrol noktası ({}) başarıyla doğrulandı. Hata oranı: 0%", checkpoint_type)
        } else {
