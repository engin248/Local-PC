...
impl ExecutionEngine {
    pub fn execute_workflow(mut task: Task, context: &ExecutionContext) -> Result<String, String> {
        ...
        AuditLogger::log_event(task_id, "info", "Gate 1/8: Planning Gate başlatılıyor...", Some("Planning Gate"), Some("gate_start"), None)?;
        if let Err(e) = PlanningGate::validate_planning(task_id) {
            AuditLogger::log_event(task_id, "error", &format!("Planning Gate BAŞARISIZ: {}", e), Some("Planning Gate"), Some("gate_fail"), None)?;
            return Err(e);
        }
        CheckpointManager::verify_checkpoint(task_id, None, "Planning Verification", true)?;
        AuditLogger::log_event(task_id, "info", "Planning Gate BAŞARIYLA GEÇİLDİ.", Some("Planning Gate"), Some("gate_success"), None)?;
        ...
        let existing_nodes: Vec<crate::core::decision_tree_builder::DecisionNode> = stmt.query_map(params![task_id], |row| {
        ...
        for node in &nodes {
            ...
            AuditLogger::log_event(task_id, "info", &format!("Düğüm {} işleniyor (Grup: {})...", node_id, node.breakdown_id), None, Some("node_start"), None)?;
            ...
            if !context.allowed_actions.contains(&action.to_string()) && matches!(context.run_mode, RunMode::ApprovedExecution | RunMode::ReadOnly) {
                let err = format!("HATA: Yürütme bağlamı bu yetkilendirme aksiyonuna izin vermiyor: {}", action);
                AuditLogger::log_event(task_id, "error", &err, Some("Authority Gate"), Some("gate_fail"), None)?;
                return Err(err);
            }
            ...
            if context.read_only && write_like_action {
                let err = format!("HATA: Read-only execution context yazma/yan etkili aksiyonu engelledi: {}", action);
                AuditLogger::log_event(task_id, "error", &err, Some("Authority Gate"), Some("read_only_blocked"), None)?;
                return Err(err);
            }

            // --- GATE 2: AUTHORITY GATE ---
            task.current_gate = Some("Authority Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 2/8: Authority Gate başlatılıyor (Düğüm: {})...", node_id), Some("Authority Gate"), Some("gate_start"), None)?;
            if let Err(e) = AuthorityRouter::route_and_validate(node) {
                AuditLogger::log_event(task_id, "error", &format!("Authority Gate BAŞARISIZ: {}", e), Some("Authority Gate"), Some("gate_fail"), None)?;
                return Err(e);
            }
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Authority Verification", true)?;
            AuditLogger::log_event(task_id, "info", "Authority Gate BAŞARIYLA GEÇİLDİ.", Some("Authority Gate"), Some("gate_success"), None)?;

            // --- GATE 3: STATEMENT GATE ---
            task.current_gate = Some("Statement Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 3/8: Statement Gate başlatılıyor (Düğüm: {})...", node_id), Some("Statement Gate"), Some("gate_start"), None)?;
            ...
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Statement Verification", true)?;
            AuditLogger::log_event(task_id, "info", "Statement Gate BAŞARIYLA GEÇİLDİ.", Some("Statement Gate"), Some("gate_success"), None)?;

            // --- GATE 4: ALTERNATIVE GATE ---
            task.current_gate = Some("Alternative Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 4/8: Alternative Gate başlatılıyor (Düğüm: {})...", node_id), Some("Alternative Gate"), Some("gate_start"), None)?;
            let alts = AlternativeAnalyzer::generate_alternatives(node_id)?;
            if alts.len() < 3 {
                let err = "HATA: Kritik karar için en az 3 alternatif alternatif analiz edilmelidir!".to_string();
                AuditLogger::log_event(task_id, "error", &format!("Alternative Gate BAŞARISIZ: {}", err), Some("Alternative Gate"), Some("gate_fail"), None)?;
                return Err(err);
            }
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Alternative Verification", true)?;
            AuditLogger::log_event(task_id, "info", "Alternative Gate BAŞARIYLA GEÇİLDİ.", Some("Alternative Gate"), Some("gate_success"), None)?;

            // --- GATE 5: RISK GATE ---
            task.current_gate = Some("Risk Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 5/8: Risk Gate başlatılıyor (Düğüm: {})...", node_id), Some("Risk Gate"), Some("gate_start"), None)?;
            let risk = RiskEngine::assess_risk(task_id, node_id, action)?;
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Risk Verification", true)?;
            AuditLogger::log_event(task_id, "info", &format!("Risk Gate BAŞARIYLA GEÇİLDİ. Belirlenen Risk: {}", risk.risk_level), Some("Risk Gate"), Some("gate_success"), None)?;

            // --- GATE 6: APPROVAL GATE ---
            task.current_gate = Some("Approval Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 6/8: Approval Gate başlatılıyor (Düğüm: {})...", node_id), Some("Approval Gate"), Some("gate_start"), None)?;
            ...
            let rules_config = rules
                ...
                .ok_or_else(|| format!("Risk seviyesi için kural bulunamadı: {}", risk.risk_level))?;
            ...
            if requires_approval {
                ...
                if requires_double_check {
                    ...
                    if !std::path::Path::new(&rules_path).exists() {
                        ...
                        AuditLogger::log_event(task_id, "error", &format!("Approval Gate BAŞARISIZ: {}", err), Some("Approval Gate"), Some("policy_check_failed"), None)?;
                        return Err(err);
                    }

                    // 2. Lock: User / Admin Approval Check (Ensure at least 2 distinct user/admin approval signatures exist in DB)
                    let approved_count: i32 = conn.query_row(
                    ...
                    let authorized_approved_count: i32 = conn.query_row(
                        "SELECT COUNT(DISTINCT approver_id) FROM approvals 
                         WHERE task_id = ?1
                         AND decision_node_id = ?2
                         AND action = ?3
                    ...
                    if approved_count < 2 || authorized_approved_count < 2 {
                        ...
                        AuditLogger::log_event(task_id, "error", &format!("Approval Gate BAŞARISIZ: {}", err), Some("Approval Gate"), Some("user_check_failed"), None)?;
                        return Err(err);
                    }
                    ...
                    if planning_status != "planning_complete" {
                        let err = "HATA: Üçlü Kilit - Geri alma ve planlama doğrulaması başarısız!".to_string();
                        AuditLogger::log_event(task_id, "error", &format!("Approval Gate BAŞARISIZ: {}", err), Some("Approval Gate"), Some("rollback_check_failed"), None)?;
                        return Err(err);
                    }

                    AuditLogger::log_event(task_id, "info", "Üçlü Kilit Güvenlik Modeli başarıyla doğrulandı.", Some("Approval Gate"), Some("double_check_passed"), None)?;
                } else {
                    ...
                    if !approved {
                        ...
                        AuditLogger::log_event(task_id, "error", &format!("Approval Gate BAŞARISIZ: {}", err), Some("Approval Gate"), Some("gate_fail"), None)?;
                        return Err(err);
                    }
                }
            } else {
                AuditLogger::log_event(task_id, "info", "Düşük riskli işlem: Onay gerekmiyor.", Some("Approval Gate"), Some("gate_bypass"), None)?;
            }
            
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Approval Verification", true)?;
            AuditLogger::log_event(task_id, "info", "Approval Gate BAŞARIYLA GEÇİLDİ.", Some("Approval Gate"), Some("gate_success"), None)?;

            // --- GATE 7: ROLLBACK GATE ---
            task.current_gate = Some("Rollback Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 7/8: Rollback Gate başlatılıyor (Düğüm: {})...", node_id), Some("Rollback Gate"), Some("gate_start"), None)?;
            
            let target_path = Self::resolve_target_path(&plan.impact_area)?;
            let target_type = if target_path.ends_with(".db") || target_path.contains("storage/app.db") {
                "sqlite"
            ...
            AuditLogger::log_event(task_id, "info", &format!("Snapshot yedek başarıyla oluşturuldu: {}", snapshot_path), Some("Rollback Gate"), Some("snapshot_created"), None)?;
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Rollback Verification", true)?;
            AuditLogger::log_event(task_id, "info", "Rollback Gate BAŞARIYLA GEÇİLDİ.", Some("Rollback Gate"), Some("gate_success"), None)?;

            // --- GATE 8: TEST GATE ---
            task.current_gate = Some("Test Gate".to_string());
            AuditLogger::log_event(task_id, "info", &format!("Gate 8/8: Test Gate başlatılıyor (Düğüm: {})...", node_id), Some("Test Gate"), Some("gate_start"), None)?;
            
            if plan.test_criteria.is_empty() {
                let err = "HATA: Test Gate için gerçek test kriteri bulunamadı.".to_string();
                AuditLogger::log_event(task_id, "error", &err, Some("Test Gate"), Some("gate_fail"), None)?;
                return Err(err);
            }
            ...
            for criterion in &plan.test_criteria {
                ...
                if let Err(e) = TestManager::run_test(task_id, &test_cmd, expected, "pending") {
                    AuditLogger::log_event(task_id, "error", &format!("Test Gate BAŞARISIZ: {}", e), Some("Test Gate"), Some("gate_fail"), None)?;
                    RollbackManager::trigger_rollback(task_id)?;
                    return Err(e);
                }
            }
            CheckpointManager::verify_checkpoint(task_id, Some(node_id), "Test Verification", true)?;
            AuditLogger::log_event(task_id, "info", "Test Gate BAŞARIYLA GEÇİLDİ.", Some("Test Gate"), Some("gate_success"), None)?;
        ...
        AuditLogger::log_event(task_id, "info", "TÜM KAPILAR BAŞARIYLA GEÇİLDİ. OPERASYON TAMAMLANDI.", None, Some("task_finished"), None)?;
...
mod tests {
    ...
    fn test_triple_lock_execution_workflow() {
        ...
        let _ = conn.execute("DELETE FROM dependency_assessments WHERE task_id = ?1", params![task_id]);
        let _ = conn.execute("DELETE FROM task_breakdown WHERE task_id = ?1", params![task_id]);
        let _ = conn.execute("DELETE FROM decision_nodes WHERE task_id = ?1", params![task_id]);

        // Establish real file for physical test gate
        let root = crate::core::dependency_analyzer::DependencyAnalyzer::get_project_root().unwrap();
        let test_log_path = root.join("storage").join("logs").join("integration_test_target.txt");
        let _ = std::fs::create_dir_all(test_log_path.parent().unwrap());
        let _ = std::fs::write(&test_log_path, b"Initial contents for rollback snapshot integration test");
        ...
        let plan = PlanningStandardInput {
            ...
            alternatives: vec!["Alt 1".to_string(), "Alt 2".to_string(), "Alt 3".to_string()],
            ...
            operation_plan: "Create file and perform write operation".to_string(),
        ...
        let breakdowns = crate::core::task_decomposer::TaskDecomposer::decompose_task(task_id, "Write changes to integration test target.").unwrap();
        let nodes = crate::core::decision_tree_builder::DecisionTreeBuilder::build_tree(task_id, &breakdowns).unwrap();
        assert!(!nodes.is_empty(), "Decision tree nodes should be generated");

        // Load authority matrix for dynamic mapping in tests
        let matrix_path = crate::core::dependency_analyzer::DependencyAnalyzer::get_config_path("authority_matrix.json").unwrap();
        ...
        for node in &nodes {
            ...
            let action = if let Some(mappings) = matrix.get("level_mappings") {
                let level_str = node.level.to_string();
                mappings.get(&level_str)
                    .and_then(|v| v.as_str())
                    .unwrap_or_else(|| {
                        mappings.get("default").and_then(|v| v.as_str()).unwrap_or("terminal_command")
                    })
            } else {
                "terminal_command"
            };
            ...
            let approval1_id = ApprovalManager::request_approval(task_id, Some(node_id), action, "high").unwrap();
            conn.execute(
            ...
            let approval2_id = ApprovalManager::request_approval(task_id, Some(node_id), action, "high").unwrap();
            conn.execute(
        ...
        assert!(result.is_ok(), "Workflow execution should succeed with all 8 gates passing: {:?}", result.err());

        // Clean up
        let _ = std::fs::remove_file(test_log_path);
        let plan_file = root.join("storage").join("backups").join(format!("plan_{}.json", task_id));
