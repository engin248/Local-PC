...
pub struct SystemConnectorConfig {
    ...
    pub approval_required_actions: Option<Vec<String>>,
    pub rollback_required_actions: Option<Vec<String>>,
    pub test_required_actions: Option<Vec<String>>,
...
pub struct AIProviderConfig {
    ...
    pub max_payload_policy: Option<serde_json::Value>,
    pub sensitive_data_policy: Option<serde_json::Value>,
}
...
impl DependencyAnalyzer {
    pub fn get_project_root() -> Result<PathBuf, String> {
        if let Some(root) = Self::find_development_root() {
            return Ok(root);
        }

        let runtime_root = Self::get_runtime_app_root()?;
        Self::seed_runtime_config(&runtime_root)?;
        Ok(runtime_root)
    }
    ...
    fn get_runtime_app_root() -> Result<PathBuf, String> {
    ...
    fn runtime_config_has_required_schema(filename: &str, target: &Path) -> bool {
        ...
        match filename {
            "ai_providers.json" => value
                .as_array()
                .map(|items| {
                    items.iter().all(|item| {
                        item.get("network_required").is_some()
                ...
                .unwrap_or(false),
            "system_connectors.json" => value
                .as_array()
                .map(|items| {
                    items.iter().all(|item| {
                        item.get("allowed_actions").is_some()
                            && item.get("approval_required_actions").is_some()
    ...
    fn embedded_config_files() -> [(&'static str, &'static str); 11] {
        [
            (
                "asker_motoru_bridge.json",
                include_str!("../../../config/asker_motoru_bridge.json"),
            ),
            (
                "ai_providers.json",
            ...
            (
                "approval_rules.json",
                include_str!("../../../config/approval_rules.json"),
            ),
            (
                "authority_matrix.json",
                include_str!("../../../config/authority_matrix.json"),
            ),
            (
                "decision_principles.json",
                include_str!("../../../config/decision_principles.json"),
            ),
            (
                "planning_standard.json",
                include_str!("../../../config/planning_standard.json"),
            ),
            (
                "risk_rules.json",
                include_str!("../../../config/risk_rules.json"),
            ),
            (
                "rollback_rules.json",
                include_str!("../../../config/rollback_rules.json"),
            ),
            (
            ...
            (
                "skill_library.json",
                include_str!("../../../config/skill_library.json"),
            ),
            (
                "system_rules.json",
    ...
    pub fn analyze_system_connector_dependency(
        ...
        let api_key_required = connector
            .api_key_env
            .as_ref()
            .map(|s| !s.is_empty())
            .unwrap_or(false);
        ...
        let approval_required = live_system
            || dependency_level == "critical"
            || permissions
                .iter()
                .any(|p| p.contains("write") || p.contains("delete") || p.contains("terminal"));
    ...
    pub fn analyze_technology_dependency(
        ...
        let assessment = DependencyAssessment {
            ...
            network_required: matches!(
                technology_name.to_lowercase().as_str(),
                "postgresql" | "supabase"
            ),
    ...
    pub fn analyze_dependency(task_id: &str, connector_id: &str) -> Result<String, String> {
        ...
        if let Ok(assessment) =
            Self::analyze_system_connector_dependency(task_id, connector_id, &config_path)
        {
...
mod tests {
    ...
    fn test_infer_level() {
        assert_eq!(
            DependencyAnalyzer::infer_level("unsupported_connector", None, false),
            "critical"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("folder", None, false),
            "low"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("sqlite", None, false),
            "low"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("local_api", None, false),
            "medium"
        );
        assert_eq!(DependencyAnalyzer::infer_level("api", None, false), "high");
        assert_eq!(
            DependencyAnalyzer::infer_level("live_api", None, false),
            "critical"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("terminal", None, false),
            "critical"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("api", None, true),
            "critical"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("api", Some("medium"), false),
            "medium"
        );
        assert_eq!(
            DependencyAnalyzer::infer_level("api", Some("invalid_val"), false),
            "critical"
        );
    }
    ...
    fn test_analyze_technology_dependency() {
        ...
        let res_sub =
            DependencyAnalyzer::analyze_technology_dependency("test_task", "supabase").unwrap();
    ...
    fn test_system_connector_scenarios() {
        ...
        let exists_folder_str = exists_folder
            .to_string_lossy()
            .to_string()
            .replace("\\", "/");
        ...
        let exists_sqlite_str = exists_sqlite_file
            .to_string_lossy()
            .to_string()
            .replace("\\", "/");
        ...
        let res = DependencyAnalyzer::analyze_system_connector_dependency(
            "test_task",
            "test_unsupported_connector",
            &config_path_str,
        )
        .unwrap();
        ...
        let err = DependencyAnalyzer::analyze_system_connector_dependency(
            "test_task",
