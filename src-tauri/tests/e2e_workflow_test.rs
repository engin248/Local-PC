...
fn e2e_swarm_allocate_and_asker_scan() {
    ...
    assert_eq!(report.contract.health.path, "/health");
    assert_eq!(report.contract.status.path, "/status");
    assert_eq!(report.contract.events.path, "/events");
    assert_eq!(report.contract.command.path, "/command");
}
